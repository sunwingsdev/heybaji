...
const AddWithdrawMethodForm = ({ setAddNewMethod }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (file) {
        ...
        if (filePath) {
          ...
          if (response.ok && data.insertedId) {
            ...
            setFile(null);
            setInstruction("");
            setAddNewMethod(false); // Go back to the list view
          } else {
            addToast(data.error || "Failed to add payment method.", {
              appearance: "error",
              autoDismiss: true,
            });
          }
      ...
      addToast("Error adding payment method.", {
        appearance: "error",
        autoDismiss: true,
      });
    } finally {
...
export default AddWithdrawMethodForm;